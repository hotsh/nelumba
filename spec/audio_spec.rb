...
require_relative '../lib/lotus/audio.rb'

describe Lotus::Audio do
  describe "#initialize" do
    it "should store an author" do
      author = mock('author')
      Lotus::Audio.new(:author => author).author.must_equal author
    end

    it "should store the stream" do
      Lotus::Audio.new(:stream => "txt").stream.must_equal "txt"
    end

    it "should store the embed code" do
      Lotus::Audio.new(:embed_code => "txt").embed_code.must_equal "txt"
    end

    it "should store content" do
      Lotus::Audio.new(:content => "txt").content.must_equal "txt"
    end

    it "should store the published date" do
      time = mock('date')
      Lotus::Audio.new(:published => time).published.must_equal time
    end

    it "should store the updated date" do
      time = mock('date')
      Lotus::Audio.new(:updated => time).updated.must_equal time
    end

    it "should store a display name" do
      Lotus::Audio.new(:display_name => "url")
                        .display_name.must_equal "url"
    end

    it "should store a summary" do
      Lotus::Audio.new(:summary => "url").summary.must_equal "url"
    end

    it "should store a url" do
      Lotus::Audio.new(:url => "url").url.must_equal "url"
    end

    it "should store an id" do
      Lotus::Audio.new(:uid => "id").uid.must_equal "id"
    end
  end

  describe "#to_hash" do
    it "should contain the stream" do
      Lotus::Audio.new(:stream => "txt")
                  .to_hash[:stream].must_equal "txt"
    end

    it "should contain the embed code" do
      Lotus::Audio.new(:embed_code => "txt")
                  .to_hash[:embed_code].must_equal "txt"
    end

    it "should contain the content" do
      Lotus::Audio.new(:content => "Hello")
                        .to_hash[:content].must_equal "Hello"
    end

    it "should contain the author" do
      author = mock('Lotus::Author')
      Lotus::Audio.new(:author => author).to_hash[:author].must_equal author
    end

    it "should contain the uid" do
      Lotus::Audio.new(:uid => "Hello").to_hash[:uid].must_equal "Hello"
    end

    it "should contain the url" do
      Lotus::Audio.new(:url => "Hello").to_hash[:url].must_equal "Hello"
    end

    it "should contain the summary" do
      Lotus::Audio.new(:summary=> "Hello")
                 .to_hash[:summary].must_equal "Hello"
    end

    it "should contain the display name" do
      Lotus::Audio.new(:display_name => "Hello")
                 .to_hash[:display_name].must_equal "Hello"
    end

    it "should contain the published date" do
      date = mock('Time')
      Lotus::Audio.new(:published => date).to_hash[:published].must_equal date
    end

    it "should contain the updated date" do
      date = mock('Time')
      Lotus::Audio.new(:updated => date).to_hash[:updated].must_equal date
    end
  end
  ...
  describe "#to_json" do
    before do
      ...
      @note = Lotus::Audio.new :content      => "Hello",
                               :author       => author,
                               :embed_code   => "code",
                               :stream       => "foo",
                               :summary      => "foo",
