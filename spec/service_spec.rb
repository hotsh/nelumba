...
require_relative '../lib/lotus/service.rb'

describe Lotus::Service do
  describe "#initialize" do
    it "should store an author" do
      author = mock('author')
      Lotus::Service.new(:author => author).author.must_equal author
    end

    it "should store content" do
      Lotus::Service.new(:content => "txt").content.must_equal "txt"
    end

    it "should store the published date" do
      time = mock('date')
      Lotus::Service.new(:published => time).published.must_equal time
    end

    it "should store the updated date" do
      time = mock('date')
      Lotus::Service.new(:updated => time).updated.must_equal time
    end

    it "should store a display name" do
      Lotus::Service.new(:display_name => "url")
                        .display_name.must_equal "url"
    end

    it "should store a summary" do
      Lotus::Service.new(:summary => "url").summary.must_equal "url"
    end

    it "should store a url" do
      Lotus::Service.new(:url => "url").url.must_equal "url"
    end

    it "should store an id" do
      Lotus::Service.new(:uid => "id").uid.must_equal "id"
    end
  end

  describe "#to_hash" do
    it "should contain the content" do
      Lotus::Service.new(:content => "Hello")
                        .to_hash[:content].must_equal "Hello"
    end

    it "should contain the author" do
      author = mock('Lotus::Person')
      Lotus::Service.new(:author => author).to_hash[:author].must_equal author
    end

    it "should contain the uid" do
      Lotus::Service.new(:uid => "Hello").to_hash[:uid].must_equal "Hello"
    end

    it "should contain the url" do
      Lotus::Service.new(:url => "Hello").to_hash[:url].must_equal "Hello"
    end

    it "should contain the summary" do
      Lotus::Service.new(:summary=> "Hello")
                 .to_hash[:summary].must_equal "Hello"
    end

    it "should contain the display name" do
      Lotus::Service.new(:display_name => "Hello")
                 .to_hash[:display_name].must_equal "Hello"
    end

    it "should contain the published date" do
      date = mock('Time')
      Lotus::Service.new(:published => date).to_hash[:published].must_equal date
    end

    it "should contain the updated date" do
      date = mock('Time')
      Lotus::Service.new(:updated => date).to_hash[:updated].must_equal date
    end
  end
  ...
  describe "#to_json" do
    before do
      ...
      @note = Lotus::Service.new :content      => "Hello",
                               :author       => author,
                               :uid          => "id",
                               :url          => "url",
                               :title        => "title",
